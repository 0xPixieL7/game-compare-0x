...
class UserPreferencesManager {
  ...
  private savePreferences(): void {
    ...
    try {
      const cookieOptions: Cookies.CookieAttributes = {
        expires: this.isGuest ? GUEST_SESSION_DURATION / (60 * 24) : PERMANENT_DURATION, // Convert minutes to days for guests
        secure: window.location.protocol === 'https:',
        sameSite: 'lax'
      }

      Cookies.set(PREFERENCES_COOKIE_KEY, JSON.stringify(this.preferences), cookieOptions)
    } catch (error) {
  ...
  deleteList(listId: string): boolean {
    ...
    const index = this.preferences.lists.findIndex(list => list.id === listId)
    if (index > -1) {
  ...
  addToRecentlyViewed(gameId: number): void {
    ...
    this.preferences.recentlyViewed = this.preferences.recentlyViewed.filter(id => id !== gameId)
  ...
  getSessionInfo(): { isGuest: boolean; expiresAt?: string; duration: string } {
    const info = {
      isGuest: this.isGuest,
      duration: this.isGuest ? `${GUEST_SESSION_DURATION} minutes` : 'Permanent'
    }

    if (this.isGuest && this.preferences.lastActivity) {
      const lastActivity = new Date(this.preferences.lastActivity)
      const expiresAt = new Date(lastActivity.getTime() + GUEST_SESSION_DURATION * 60 * 1000)
...
export const getPreferencesManager = (isAuthenticated: boolean = false): UserPreferencesManager => {
  if (isAuthenticated) {
    if (!userPreferences) {
...
export const useUserPreferences = (isAuthenticated: boolean = false) => {
  ...
  return {
    ...
    addGameToList: (listId: string, gameId: number) => manager.addGameToList(listId, gameId),
    removeGameFromList: (listId: string, gameId: number) => manager.removeGameFromList(listId, gameId),
    isGameInList: (listId: string, gameId: number) => manager.isGameInList(listId, gameId),
    deleteList: (listId: string) => manager.deleteList(listId),
    renameList: (listId: string, newName: string) => manager.renameList(listId, newName),

    // Recently viewed
    addToRecentlyViewed: (gameId: number) => manager.addToRecentlyViewed(gameId),
    getRecentlyViewed: () => manager.getRecentlyViewed(),

    // Favorite genres
    addFavoriteGenre: (genre: string) => manager.addFavoriteGenre(genre),
    removeFavoriteGenre: (genre: string) => manager.removeFavoriteGenre(genre),
