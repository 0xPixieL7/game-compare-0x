...
class FetchIgdbGenresCommand extends Command
{
    protected $signature = 'gc:fetch-igdb-genres
        {--limit=500 : IGDB page size (max 500)}
        {--out=storage/igdb-dumps/genres.csv : Output CSV path}
    ';

    protected $description = 'Fetch IGDB genre metadata via v4 API and export to CSV';
    ...
    public function handle(): int
    {
        ...
        $this->info('Fetched IGDB token. Downloading genres...');

        $rows = $this->fetchGenres($token, $clientId, $limit);

        if ($rows === []) {
            $this->warn('No genre rows returned.');

            return static::FAILURE;
        }

        $written = $this->writeCsv($outPath, $rows);
        $this->info("Wrote {$written} genre rows to {$outPath}");
     ...
     */
    protected function fetchGenres(string $token, string $clientId, int $limit): array
    {
        $results = [];
        $offset = 0;
        ...
        while ($maxPages-- > 0) {
            ...
            if (! $response->successful()) {
                $this->error('Genres request failed: '.$response->body());
            ...
            $this->info(sprintf('Fetched %d genre rows (offset %d)', $count, $offset));
