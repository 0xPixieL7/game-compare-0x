...
impl Db {
    ...
    pub async fn bulk_insert_prices(&self, rows: &[PriceRow]) -> Result<()> {
        ...
        let mut laravel_written = 0;
        let mut legacy_written = 0;
        ...
        if has_video_game_prices {
            ...
            if !laravel_rows.is_empty() {
                ...
                qb.push_values(laravel_rows.iter(), |mut b, r| {
        ...
        // CRITICAL FIX: ALWAYS write to legacy prices table (removed early return)
        // Pre-ensure partitions for distinct months
        {
            use std::collections::HashSet;
            ...
            for (y, m) in months {
                ...
                sqlx::query("SELECT ensure_price_partition($1::timestamptz)")
                    .bind(first)
                    .execute(&mut *tx)
                    .await
