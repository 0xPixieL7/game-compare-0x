...
pub async fn run(cfg: DbSchemaAuditConfig) -> Result<()> {
    ...
    let connect_opts = raw_url.parse::<PgConnectOptions>()?.statement_cache_capacity(0);

    let max_conns = cfg.max_connections
        .or_else(|| {
            envutil::env_opt("SCHEMA_AUDIT_MAX_CONNS").and_then(|v| v.parse::<u32>().ok())
        })
        .unwrap_or(4);
    ...
    let pool: Pool<Postgres> = PgPoolOptions::new()
        ...
        .after_connect(|conn, _| {
            Box::pin(async move {
                let _ = sqlx
                    ::query("SET application_name = 'schema_audit'")
                    .persistent(false)
                    .execute(conn).await;
                Ok(())
            })
    ...
    writeln!(out, "SCHEMA AUDIT (public) — total tables: {}", tables.len()).ok();

    for table in tables {
        let mut conn = pool.acquire().await?;

        let exists: Option<String> = sqlx
            ::query_scalar("SELECT to_regclass($1)::text")
...
async fn emit_columns(out: &mut String, table: &str, conn: &mut PgConn) -> Result<()> {
    ...
    for row in rows {
        ...
        writeln!(out, "  {name}:{dt}:{}", if nul == "YES" { "null" } else { "not-null" }).ok();
    }
    Ok(())
}
